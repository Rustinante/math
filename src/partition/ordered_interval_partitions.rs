//! Interval partitions ordered by the first element of each parititon.
...
impl<E: Integer + Copy> OrderedIntervalPartitions<E> {
    ...
    pub fn from_slice(slice: &[[E; 2]]) -> OrderedIntervalPartitions<E> {
        OrderedIntervalPartitions::from_vec(
            slice
                .iter()
            ...
            match lhs.intersect(&rhs) {
                ...
                Some(intersection) => {
                    ...
                    if lhs.get_end() > intersection.get_end() {
                        lhs_remnant = Some(ContiguousIntegerSet::new(
                            intersection.get_end() + E::one(),
                            lhs.get_end(),
                        ));
                    } else {
                        lhs_remnant = None;
                    ...
                    if rhs.get_end() > intersection.get_end() {
                        rhs_remnant = Some(ContiguousIntegerSet::new(
                            intersection.get_end() + E::one(),
                            rhs.get_end(),
                        ));
                    } else {
...
mod tests {
    ...
    fn test_ordered_interval_partitions_common_refinement() {
        ...
        test(&[[0usize, 10], [15, 23]], &[[3, 5], [20, 23]], &[
            [0, 2],
            [3, 5],
            [6, 10],
            [15, 19],
        ...
        test(&[[0usize, 10], [15, 23]], &[[3, 5], [15, 23]], &[
            [0, 2],
            [3, 5],
            [6, 10],
            [15, 23],
        ]);
        test(&[[0usize, 10], [15, 23]], &[[3, 5], [15, 20]], &[
            [0, 2],
            [3, 5],
            [6, 10],
            [15, 20],
